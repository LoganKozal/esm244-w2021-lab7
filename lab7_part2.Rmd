...
cache = TRUE so that it doesnt reread it in every time
```{r, cache=TRUE}
...
```{r}
#first \ is to escape r thinking \ means something else
...
```{r}
#NAs have a class behind the scenes
...
hobbit_word_count <- hobbit_tokens %>% 
  count(chapter, word)

```

## Remove all stopwords that exist in hobbit tokens
...
ggplot(data = top_5_words, aes(x=word, y=n)) +
  geom_col(fill ="blue") +
  facet_wrap(~chapter, scales = "free") + # free allows the axes to differ across facet_wrap
  coord_flip()
```
